...
try:
	...
	try:
		if len(nascimentomembro) != 8:
			...
			nascimentomembro = int(nascimentomembro)  # se contiver letras causa um ValueError
	...
	try:
		if len(candidaturamembro) != 8:
			...
			candidaturamembro = int(candidaturamembro)  # se contiver letras causa um ValueError
	...
	alterar = """ update membros set  nomemembro = (%s), telmembro = (%s), enderecomembro = (%s), nascimentomembro = (%s), candidaturamembro = (%s), emergenciamembro = (%s), sanguemembro = (%s)
		where idmembro = (%s)"""
	sql_data = nome, tel, endereco, nascimento, candidatura, emergencia, sangue, id_alterar
